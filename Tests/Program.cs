...
Check(SourceSelection.Select(RotationSource.Auto, false, false, false) == null, "auto waits without an active rotation");
Check(SourceSelection.Select(RotationSource.Auto, true, false, false) == RotationSource.Wrath, "auto selects Wrath");
Check(SourceSelection.Select(RotationSource.Auto, false, true, false) == RotationSource.BossMod, "auto selects BossMod");
Check(SourceSelection.Select(RotationSource.Auto, false, false, true) == RotationSource.RotationSolver, "auto selects RSR");
Check(SourceSelection.Select(RotationSource.Auto, true, false, true) == null, "auto rejects conflicting sources");
Check(SourceSelection.Select(RotationSource.Auto, true, true, true) == null, "auto rejects three sources");
Check(SourceSelection.Select(RotationSource.Wrath, false, false, true) == RotationSource.Wrath, "manual selection never falls back");
var unknownTimeGate = new CueGate();
Check(unknownTimeGate.Update(new(Direction.Rear, 0, 0, 1, 100, false), null, true, 1, 30000), "direction-only hint can chime");
Check(!unknownTimeGate.Update(new(Direction.Rear, 0, 0, 1, 100, false), null, true, 1, 34000), "direction-only hint does not repeat chime");
Check(SourceSelection.BossDirection(1) == Direction.Flank, "Reborn flank differs from Wrath enum");
Check(SourceSelection.BossDirection(2) == Direction.Rear, "Reborn rear differs from Wrath enum");
...
Check(!ForecastMath.UseAoe(2, 3), "forecast ST below Wrath threshold");
Check(ForecastMath.UseAoe(3, 3), "forecast AoE at Wrath threshold");
Check(!ForecastMath.UseAoe(9, null), "forecast nullable threshold disables AoE");
Check(!ForecastMath.UseAoe(9, 0), "forecast rejects invalid threshold");
var zero = System.Numerics.Vector2.Zero;
var aim = new System.Numerics.Vector2(0, 10);
Check(ForecastMath.Hits(2, zero, aim, new(0, 5.5f), 5, 0, 0, 1), "circle includes enemy hitbox");
Check(!ForecastMath.Hits(2, zero, aim, new(0, 6.5f), 5, 0, 0, 1), "circle excludes distant enemy");
Check(ForecastMath.Hits(4, zero, aim, new(1, 5), 0, 10, 4, 0), "line target in width");
Check(!ForecastMath.Hits(4, zero, aim, new(3, 5), 0, 10, 4, 0), "line excludes side enemy");
Check(!ForecastMath.Hits(4, zero, aim, new(0, -5), 0, 10, 4, 0), "line excludes behind enemy");
Check(ForecastMath.Hits(3, zero, aim, new(2, 5), 0, 10, 0, 0), "cone includes forward enemy");
Check(!ForecastMath.Hits(3, zero, aim, new(6, 3), 0, 10, 0, 0), "cone excludes side enemy");
Check(!ForecastMath.Hits(3, zero, zero, new(1, 1), 0, 10, 0, 0), "no cone without aim");
